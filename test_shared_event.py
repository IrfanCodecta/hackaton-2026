 ...
 def test_two_installations_share_counts_comments_and_preserve_local_data(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   async def scenario():
    ...
    for action,body in [('join',{}),('accept',{}),('join_card',{'card':'agent-marketplace'}),('comment',{'card':'agent-marketplace','text':'Private old copy'})]:
    ...
    for host in ['alice.example','bob.example']:
     ...
     for action,body in [('join',{}),('accept',{}),('join_card',{'card':'agent-marketplace'})]:await command(host,action,body)
    post=await command('alice.example','comment',{'card':'agent-marketplace','text':'Shared finding'})
    d=await command('bob.example','detail',{'card':'agent-marketplace'})
    self.assertEqual(d['members'],['@alice','@bob']);self.assertEqual(d['posts'][0]['text'],'Shared finding')
    reply=await command('bob.example','comment',{'card':'agent-marketplace','parent_id':post['id'],'text':'Shared reply'})
    d=await command('alice.example','replies',{'card':'agent-marketplace','parent_id':post['id']})
    self.assertEqual(d['posts'][0]['id'],reply['id'])
    for host in ['alice.example','bob.example']:
     d=await command(host,'state');self.assertEqual(d['cards'][0]['count'],2)
    old=await command('alice.example','detail',{'card':'agent-marketplace'},target='alice.example')
